...
use credentials::{AuthBundle, Profile, UserInfo};
use mom_types::{
    GenerateLoginCodeRequest, GenerateLoginCodeResponse, ValidateLoginCodeRequest,
...
pub async fn generate_login_code(
    ...
    log::info!(
        "Email login request received for email: {} (tenant: {})",
        req.email,
        ts.ti.tc.name
    );
    ...
    if !req.email.contains('@') || req.email.len() < 3 {
        ...
        return HttpError::with_status(StatusCode::BAD_REQUEST, "Invalid email format")
            .into_reply();
    }
    ...
    log::debug!("Login code created at {created_at} and expires at {expires_at}");
    ...
    log::debug!("Inserted {rows_affected} row(s) into email_login_codes table");
    ...
    if let Some(email_service) = &crate::impls::global_state().email_service {
        ...
        match email_service
            .send_login_code(&req.email, &code, ts.ti.tc.name.as_str())
            .await
        {
            Ok(_) => {
            ...
            Err(e) => {
                ...
                log::warn!(
                    "Continuing despite email send failure - code will be returned in response"
                );
            }
        }
    } else {
        log::warn!(
            "Email service not configured, cannot send login code to {}",
            req.email
        );
        if is_development() {
    ...
    log::info!(
        "Login code generation completed for email: {} (code expires at: {})",
        req.email,
        expires_at
    );
    FacetJson(GenerateLoginCodeResponse { code, expires_at }).into_reply()
}
...
pub async fn validate_login_code(
    ...
    log::info!(
        "Login code validation request for email: {} (tenant: {})",
        req.email,
        ts.ti.tc.name
    );
    log::debug!(
        "Validating code: {} from IP: {:?}, User-Agent: {:?}",
        req.code,
        req.ip_address,
        req.user_agent
    );
    ...
    log::debug!("Found login code entry with ID: {id} (expires at: {expires_at})");

    // Check if code has expired
    let now = OffsetDateTime::now_utc();
    if expires_at < now {
        log::warn!(
            "Login code has expired for email {} (expired at: {}, current time: {})",
            req.email,
            expires_at,
            now
        );
    ...
    log::debug!(
        "Login code is still valid (expires in: {:?})",
        expires_at - now
    );
    // Mark code as used
    ...
    log::debug!("Updated {rows_updated} row(s) in email_login_codes table");
    // First check Stripe for subscription information
    log::info!(
        "Looking up user subscription in Stripe for email: {}",
        req.email
    );
    ...
    let stripe_user_info = libstripe::load()
        ...
        .unwrap_or_else(|e| {
            log::error!(
                "Failed to lookup user in Stripe for email {}/{}",
                req.email,
                e
            );
    ...
    let auth_bundle = if let Some(user_info) = stripe_user_info {
        ...
        log::info!(
            "User {} found in Stripe with active subscription",
            req.email
        );
        ...
        log::info!(
            "No Stripe subscription found for {}, creating basic auth",
            req.email
        );
        let email_hash = {
            use std::collections::hash_map::DefaultHasher;
    ...
    log::info!(
        "Login code validation successful for email: {} (auth expires at: {})",
        req.email,
        auth_bundle.expires_at
    );
